...
```{r}
iris_raw %>% group_by(species) %>% summarise(sepal_length.mean=mean(sepal_length),
                       sepal_length.median=median(sepal_length),
                       sepal_length.min=min(sepal_length),
                       sepal_lengh.max=max(sepal_length))
...
ggplot(exprs_assay_long,aes(x=value)) + 
  geom_histogram(fill="blue", bins = 50, aes(color="gene"))+
  facet_wrap(~gene)
