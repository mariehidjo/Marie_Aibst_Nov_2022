...
```{r}
iris_features <- iris %>% mutate(iris, Species=NULL) %>% as.matrix()
...
iris_hclust_complete_labels <- cutree(hclust_iris_ward, k=3)
...
iris_hclust_average_labels <- cutree(hclust_iris_ward, k=3)
...
iris_hclust_single_labels <- cutree(hclust_iris_ward, k=3)
...
geom_point() +
geom_point(data=iris_means_centers, shape = "*", size = 10, color="black")
