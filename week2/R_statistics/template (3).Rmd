...
logcount_matrix<- as.data.frame(logcounts_matrix)
logcount_marix<- column_to_rownames(logcount_matrix, var="gene")
logcounts_matrix<- as.matrix(logcount_marix)
logcount_marix
```
...
names_mock<- cell_info$Sample[cell_info$Infection == "mock"]
gene_1_mock<- logcounts_matrix[gene_index, names_mock]

out <- t.test(gene_1_infected, gene_1_mock)
return(out$p.value)
}
